...
function ShopContent() {
  ...
  useEffect(() => {
    if (categories.length > 0 || activeSlug === 'svi') fetchProducts(activeSlug);
  }, [activeSlug, categories, fetchProducts]);

  function handleAdd(product: Product) {
    addToCart({ id: product.id, name: product.name, price: product.price, images: product.images, short_description: product.short_description });
  ...
  const SidebarContent = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
      <div style={{ fontSize: '11px', fontWeight: 600, letterSpacing: '1px', textTransform: 'uppercase', color: '#555', marginBottom: '12px', fontFamily: "'Space Grotesk', sans-serif" }}>
        Kategorije
      </div>
      <button onClick={() => handleCatClick('svi')} style={{
        display: 'flex', alignItems: 'center', gap: '12px',
        padding: '10px 14px', borderRadius: '10px', border: 'none',
        cursor: 'pointer', textAlign: 'left', transition: 'all 0.2s',
        background: activeSlug === 'svi' ? 'rgba(255,192,42,0.12)' : 'transparent',
        color: activeSlug === 'svi' ? '#ffc02a' : '#888',
        fontWeight: activeSlug === 'svi' ? 600 : 400, fontSize: '14px',
      ...
      {categories.map(cat => (
        <button key={cat.id} onClick={() => handleCatClick(cat.slug)} style={{
          display: 'flex', alignItems: 'center', gap: '12px',
          padding: '10px 14px', borderRadius: '10px', border: 'none',
          cursor: 'pointer', textAlign: 'left', transition: 'all 0.2s',
          background: activeSlug === cat.slug ? 'rgba(255,192,42,0.12)' : 'transparent',
          color: activeSlug === cat.slug ? '#ffc02a' : '#888',
          fontWeight: activeSlug === cat.slug ? 600 : 400, fontSize: '14px',
          fontFamily: "'Manrope', sans-serif",
        }}>
          <span style={{
            width: '32px', height: '32px', borderRadius: '8px', flexShrink: 0,
            ...
            <div style={{ width: '16px', height: '16px' }}>{getCategoryIcon(cat.slug)}</div>
          </span>
          <span style={{ flex: 1 }}>{cat.name}</span>
          <span style={{ fontSize: '11px', color: '#444' }}>{cat.count}</span>
        </button>
      ))}
    </div>
  );
  ...
  return (
    <>
      <div style={{ background: '#0f0f0f', color: '#f0f0f0', minHeight: '100vh' }}>
        ...
        {/* Header */}
        <section style={{ background: '#0f0f0f', paddingTop: '100px', paddingBottom: '32px', borderBottom: '1px solid #1a1a1a' }}>
          <div style={{ maxWidth: '1152px', margin: '0 auto', padding: '0 20px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: '20px', flexWrap: 'wrap' }}>
              <div>
                <h1 style={{ fontSize: 'clamp(28px, 4vw, 52px)', fontWeight: 800, letterSpacing: '-2px', color: '#ededeb', lineHeight: 1.05, fontFamily: "'Manrope', sans-serif" }}>
                  Naš asortiman
                </h1>
                <p style={{ fontSize: '15px', color: '#666', marginTop: '8px', fontFamily: "'Manrope', sans-serif" }}>
              ...
              <Link href="/" style={{ fontSize: '13px', color: '#666', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '6px', transition: 'color 0.2s', fontFamily: "'Manrope', sans-serif" }}
        ...
        <div style={{ maxWidth: '1152px', margin: '0 auto', padding: '32px 20px' }}>
          ...
          <button
            ...
            style={{
              display: 'none', width: '100%', marginBottom: '16px',
              background: '#1a1a1a', border: '1px solid #222',
              color: '#ededeb', padding: '12px 16px',
          ...
          >
            <span style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="#ffc02a" strokeWidth="1.8">
                <path d="M2 4h12M4 8h8M6 12h4" strokeLinecap="round" />
              </svg>
              {activeSlug === 'svi' ? 'Sve kategorije' : categories.find(c => c.slug === activeSlug)?.name ?? 'Kategorije'}
            </span>
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
              <path d={sidebarOpen ? 'M4 12l4-4 4 4' : 'M4 6l4 4 4-4'} />
            </svg>
          </button>

          {/* Mobilni sidebar dropdown */}
          {sidebarOpen && (
            <div className="mobile-sidebar" style={{
              display: 'none', background: '#1a1a1a', border: '1px solid #222',
          ...
          <div className="shop-layout">
            ...
            {/* Desktop sidebar */}
            <aside className="desktop-sidebar" style={{ position: 'sticky', top: '84px' }}>
              <SidebarContent />
            </aside>
                ...
                <div className="shop-grid">
                  {filtered.map(product => (
                    <div key={product.id} style={{
                      ...
                      onMouseEnter={e => { const el = e.currentTarget; el.style.borderColor = '#ffc02a'; el.style.transform = 'translateY(-3px)'; el.style.boxShadow = '0 8px 32px rgba(255,192,42,0.10)'; }}
                      onMouseLeave={e => { const el = e.currentTarget; el.style.borderColor = '#222'; el.style.transform = 'translateY(0)'; el.style.boxShadow = 'none'; }}
                    >
                          ...
                          {product.images?.[0]?.src ? (
                            ...
                            <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#444' }}>
                              <svg width="48" height="48" viewBox="0 0 48 48" fill="none" stroke="currentColor" strokeWidth="1.5">
                                <rect x="6" y="6" width="36" height="36" rx="6" /><circle cx="24" cy="24" r="8" />
                              </svg>
                            </div>
                          )}
                        </div>
                      ...
                      <div style={{ padding: '14px 16px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                        <div style={{ fontSize: '11px', color: '#ffc02a', textTransform: 'uppercase', letterSpacing: '0.6px', marginBottom: '6px', fontFamily: "'Manrope', sans-serif" }}>
                          {product.categories?.[0]?.name ?? ''}
                        </div>
                        <Link href={`/proizvodi/${product.id}`} style={{ textDecoration: 'none' }}>
                          <div style={{ fontSize: '14px', fontWeight: 700, color: '#f0f0f0', marginBottom: '6px', lineHeight: 1.3, fontFamily: "'Manrope', sans-serif" }}>
                            {product.name}
                          </div>
                        </Link>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                          <span style={{ fontSize: '18px', fontWeight: 800, color: '#ffc02a', fontFamily: "'Space Grotesk', sans-serif" }}>
                            {parseFloat(product.price).toLocaleString('sr-RS')}
                            <span style={{ fontSize: '11px', color: '#555', marginLeft: '3px', fontWeight: 400 }}>RSD</span>
                          </span>
  ...
  return (
    <Suspense fallback={
      <div style={{ background: '#0f0f0f', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ color: '#ffc02a', fontSize: '18px', fontWeight: 600, fontFamily: "'Manrope', sans-serif" }}>Učitavam shop...</div>
      </div>
    }>
      <ShopContent />
    </Suspense>
